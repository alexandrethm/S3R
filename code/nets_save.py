...
class XYZNet(nn.Module):
    ...
    def __init__(self, nb_sequences=66, nb_classes=14):
        """
        Instantiates the parameters and the modules.
        ...
        # High resolution branch ------------------------------------
        # The first module is shared by all channels
        self.high_res_1 = nn.ModuleList([
            nn.ModuleDict({
                'conv': nn.Conv1d(in_channels=22, out_channels=8, kernel_size=7, padding=3),
        ...
        # Each channel has a specific 2nd and 3rd module
        self.high_res_2 = nn.ModuleList([
            nn.ModuleDict({
                'conv': nn.Conv1d(in_channels=8, out_channels=4, kernel_size=7, padding=3),
                'activation': nn.ModuleDict[{
                    'relu': nn.ReLU(),
                    'prelu': nn.PReLU(),
                }],
        ...
        self.high_res_3 = nn.ModuleList([
            nn.ModuleDict({
                'conv': nn.Conv1d(in_channels=4, out_channels=4, kernel_size=7, padding=3),
                'activation': nn.ModuleDict[{
                    'relu': nn.ReLU(),
                    'prelu': nn.PReLU(),
                }],
        ...
        self.high_res_1 = nn.ModuleList([
            nn.ModuleDict({
                'conv': nn.Conv1d(in_channels=22, out_channels=8, kernel_size=3, padding=3),
                'activation': nn.ModuleDict[{
                    'relu': nn.ReLU(),
                    'prelu': nn.PReLU(),
                }],
        ...
        self.high_res_2 = nn.ModuleList([
            nn.ModuleDict({
                'conv': nn.Conv1d(in_channels=8, out_channels=4, kernel_size=3, padding=3),
                'activation': nn.ModuleDict[{
                    'relu': nn.ReLU(),
                    'prelu': nn.PReLU(),
                }],
        ...
        self.high_res_3 = nn.ModuleList([
            nn.ModuleDict({
                'conv': nn.Conv1d(in_channels=4, out_channels=4, kernel_size=3, padding=3),
                'activation': nn.ModuleDict[{
                    'relu': nn.ReLU(),
                    'prelu': nn.PReLU(),
                }],
        ...
        # The last layer, fully connected
        self.fc_module = nn.ModuleDict({
            'linear1': nn.Linear(in_features=30 * 3 * 12, out_features=276),
            'activation': nn.ModuleDict[{
                'relu': nn.ReLU(),
                'prelu': nn.PReLU(),
            }],
            'linear2': nn.Linear(in_features=276, out_features=nb_classes),
        })
    ...
    def forward(self, input, activation_function='relu'):
        """
        This function performs the actual computations of the network for a forward pass.
        ...
        for i in range(3):
            ...
            # Apply the 3 branches
            high = self.high_res_1[0]['conv'](channel_input)
            high = self.high_res_1[0]['activation'][activation_function](high)
            high = self.high_res_1[0]['pool'](high)
            high = self.high_res_2[i]['conv'](high)
            high = self.high_res_2[i]['activation'][activation_function](high)
            high = self.high_res_2[i]['pool'](high)
            high = self.high_res_3[i]['conv'](high)
            high = self.high_res_3[i]['activation'][activation_function](high)
            high = self.high_res_3[i]['pool'](high)

            low = self.low_res_1[0]['conv'](channel_input)
            low = self.low_res_1[0]['activation'][activation_function](low)
            low = self.low_res_1[0]['pool'](low)
